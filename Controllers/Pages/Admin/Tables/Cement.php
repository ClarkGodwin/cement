...
$cements = $DB->find_all(); 
$standards = $DB->set_table_name('standard')->find_all();
...
$cement_detailed = $DB->set_table_name('cement')->find($id); 
        ...
        <tbody>
                <?php foreach($cements as $cement): ?>
                        <tr>
                                ...
                                <td>
                                        <?= ($cement['description'] != null)? ucfirst(substr($cement['description'], 0, 30). '...'):'No description' ?> 
                                </td>
...
<section class="modal hide_modal">
        <?php $related_standards = $DB->set_table_name('standard')->find_with_column('cement_id', $cement_detailed['id']);
        if($action == 'details'){ ?>
                <div>
                        ...
                        <p><h4>Déscription</h4> : <?= ($cement_detailed['description'] != null)? ucfirst($cement_detailed['description']):'No description' ?></p>
                        <p><h4>Quantité</h4> : <?= $cement_detailed['quantity'] ?> sacs de 50 kg</p>
                        <?php if(count($related_standards) == 0){?>
                ...
                <div>
                        ...
                        <form action="" id="modifying_form">
                                ...
                                <?php }else{ ?>
                                        ...
                                        <select name="standards[]" id="standards" multiple>
                                                <?php foreach($standards as $standard): ?>
                                                        <option value="<?= $standard['id']?>" 
                                                        <?= ($standard['cement_id'] == $cement_detailed['id'])?'selected':'' ?>>
                                                                <?= $standard['name'] ?>
                                                        </option>
                                                <?php endforeach ?>
                                        </select>
